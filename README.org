...
Branches that can be proven inactive from literal boolean expressions in
=when= statements are displayed using =font-lock-comment-face=.  Set
=odin-ts-mode-fontify-inactive-when-blocks= to =nil= to disable this.
